...
export const InvestorProfile: React.FC = () => {
  ...
  const [investor, setInvestor] = useState<Investor | null>(() => {
    if (currentUser?.role === 'investor' && currentUser.id === id) {
      return currentUser;
    }

    return (findUserById(id || '') as Investor | null) || null;
  });
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      const cachedProfile = findUserById(id || '') as Investor | null;
      if (cachedProfile) {
        setInvestor(cachedProfile);
        return;
      }
      ...
      try {
        const response = await api.get<Investor>(`/users/${id}`);
