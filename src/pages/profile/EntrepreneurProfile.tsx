...
export const EntrepreneurProfile: React.FC = () => {
  ...
  const [entrepreneur, setEntrepreneur] = useState<Entrepreneur | null>(() => {
    if (currentUser?.role === 'entrepreneur' && currentUser.id === id) {
      return currentUser;
    }

    return (findUserById(id || '') as Entrepreneur | null) || null;
  });
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      if (currentUser?.role === 'entrepreneur' && currentUser.id === id) {
        setEntrepreneur(currentUser);
        return;
      }

      const cachedProfile = findUserById(id || '') as Entrepreneur | null;
      if (cachedProfile) {
        setEntrepreneur(cachedProfile);
        return;
      }
      ...
      try {
        const response = await api.get<Entrepreneur>(`/users/${id}`);
